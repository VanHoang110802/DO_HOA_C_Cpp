...
double x1, y5, x2, y2, ang[2], vec[2];
void far *buf1, *buf2;
char *mess[2] = {"Den luot nguoi ben trai di", "Den luot nguoi ben phai di"};
int gio;
...
void vedb1()
{
  ...
  setcolor(LIGHTGRAY);
  for (y=-3; y<=3; y++)
    line(x1-10*cos(RADS*ang[0]), y5 - 5 + 10*sin(RADS*ang[0])+y,
         x1+20*cos(RADS*ang[0]), y5 - 5 - 20*sin(RADS*ang[0])+y);
  setfillstyle(SOLID_FILL, CYAN);
  bar(x1-4, y5-20, x1+4, y5+20);
}
...
void screen()
{
  ...
  x1 = rand() % getmaxx()/8+20;
  y5 = 1;
  while (getpixel(x1,y5) != YELLOW)
    y5++;
  x2 = getmaxx() - rand()%getmaxx()/8-20;
  y2 = 1;
...
void message(int player)
{
  ...
  do {
    ...
    if (c != 13)
    {
      ...
      switch(player)
      {
        case 0 : putimage(x1-20, y5-30, buf1, COPY_PUT);
                 vedb1();
                 break;
...
int shoot1()
{
  ...
  while (x>0 && (x<getmaxx()-1) && y>0 && (y<getmaxy()-1) && OK)
  {
    ...
    x += 1.0 + gio / 100 ;
    t += 0.1;
    y -= (vec1*sin(RADS*ang[0]) - g*t+gio)/(vec1*cos(RADS*ang[0]));
  }
  setcolor(BLUE);
  setfillstyle(SOLID_FILL, BLUE);
  fillellipse(x, y, 6, 6);
  if (abs(x-x2) < 20 && abs(y-y2)<40)
    return 1;
  else
    return 0;
}
...
int shoot2()
{
  ...
  while (x>0 && (x<getmaxx()-1) && y>0 && (y<getmaxy()-1) && OK)
  {
    ...
    x -= 1.0 - gio/100;
    t += 0.1;
    y -= (vec2*sin(RADS*ang[1]) - g*t-gio)/(vec2*cos(RADS*ang[1]));
  }
  setcolor(BLUE);
  setfillstyle(SOLID_FILL, BLUE);
  fillellipse(x, y, 6, 6);
  if (abs(x-x1) < 20 && abs(y-y5)<40)
    return 1;
  else
  return 0;
}
...
void run()
{
  ...
  screen();
  size = imagesize(x1-20, y5-30, x1+20, y5+30);
  buf1 = malloc(size);
  buf2 = malloc(size);
  getimage(x1-20, y5-30, x1+20, y5+30, buf1);
  ...
  setcolor(YELLOW);
  itoa(abs(gio), s, 10);
  outtextxy((getmaxx()-textwidth(s))/2, 100, s);
  line((getmaxx()-gio)/2 , 110, (getmaxx()+gio)/2, 110);
  if (gio>0)
  {
    moveto((getmaxx()+gio)/2, 110);
    linerel(-6, -3);
    moveto((getmaxx()+gio)/2, 110);
    linerel(-6, +3);
  }
  ...
  do {
    ...
    if (!done)
    {
      ...
      {
        setcolor(YELLOW);
        outtextxy(100, 140, "Player Two win");
      }
    }
    else
    {
      setcolor(YELLOW);
      outtextxy(100, 140, "Player One win");
    }
    if (done)
      outtextxy(100, 150, "Press any key to continue or ESCAPE to quit");
  } while (!done);
  free(buf1);
  free(buf2);
}
